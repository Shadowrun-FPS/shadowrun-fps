...
export default function TournamentDetailsPage() {
  ...
  useEffect(() => {
    const fetchTeams = async () => {
      ...
      try {
        ...
        safeLog.error("Tournament page user teams fetch failed:", error);
      }
    };
    ...
    const checkAdminStatus = async () => {
      if (session?.user?.id) {
        try {
          ...
          safeLog.error("Tournament page permissions check failed:", error);
  ...
  const unregisterTeam = async (teamId: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        safeLog.error("Tournament unregister error response:", data);
      ...
      safeLog.error("Tournament unregister team failed:", error);
      setUnregisterError(
  ...
  const handleFillTournament = async () => {
    ...
    try {
      ...
      safeLog.error("Tournament fill failed:", error);
      toast({
        title: "Error",
        description:
  ...
  const handleClearTournament = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        safeLog.error("Tournament clear error details:", responseData);
  ...
  const handleLaunchTournament = async () => {
    ...
    try {
      ...
      safeLog.error("Tournament launch failed:", error);
      toast({
        title: "Error",
        description:
  ...
  const handleResetTournament = async () => {
    ...
    try {
      ...
      safeLog.error("Tournament reset failed:", error);
      toast({
        title: "Error",
        description:
          error instanceof Error ? error.message : "An error occurred",
      ...
      <div className="min-h-screen bg-background">
        <main className="mx-auto max-w-screen-xl px-4 py-6 sm:px-6 sm:py-8 lg:px-8 lg:py-10 xl:px-12">
          <div className="space-y-6">
            ...
            <Skeleton className="h-32 w-full rounded-2xl border-2" />
            <Skeleton className="h-24 w-full rounded-2xl border-2" />
            <Skeleton className="h-11 w-full max-w-lg rounded-xl" />
            <Skeleton className="min-h-[280px] w-full rounded-2xl border-2" />
          </div>
        </main>
      </div>
    );
          ...
          <Card className="border-2 border-dashed border-border/60">
            <CardContent className="flex flex-col items-center justify-center py-12 sm:py-16">
              ...
              <p className="mt-2 max-w-md text-center text-muted-foreground">
                The tournament you&apos;re looking for doesn&apos;t exist or may
                have been removed.
              </p>
              <Button className="mt-6 rounded-full" asChild>
                <Link href="/tournaments/overview">Back to tournaments</Link>
              </Button>
            </CardContent>
          </Card>
        </main>
      ...
      <main className="mx-auto max-w-screen-xl px-4 py-6 sm:px-6 sm:py-8 lg:px-8 lg:py-10 xl:px-12">
        <TournamentDetailHero
          tournament={{
            ...
            registeredTeamsCount: tournament.registeredTeams.length,
            maxTeams: tournament.maxTeams || 8,
          }}
          onBack={() => router.push("/tournaments/overview")}
          showEdit={Boolean(isAdmin || canManage)}
        ...
        <Tabs
          ...
          className="mb-6"
        >
          <TabsList className="mb-6 grid h-auto w-full grid-cols-3 gap-1 rounded-xl border border-border/60 bg-muted/20 p-1">
            <TabsTrigger
              value="bracket"
              className="min-h-[44px] touch-manipulation rounded-lg py-2.5 text-sm data-[state=active]:bg-background data-[state=active]:shadow-sm sm:text-base"
            >
              <GitBranch className="mr-1.5 h-4 w-4 shrink-0 sm:mr-2" aria-hidden />
              Bracket
            </TabsTrigger>
            <TabsTrigger
              value="standings"
              className="min-h-[44px] touch-manipulation rounded-lg py-2.5 text-sm data-[state=active]:bg-background data-[state=active]:shadow-sm sm:text-base"
            >
              <Trophy className="mr-1.5 h-4 w-4 shrink-0 sm:mr-2" aria-hidden />
              Standings
            </TabsTrigger>
            <TabsTrigger
              value="teams"
              className="min-h-[44px] touch-manipulation rounded-lg py-2.5 text-sm data-[state=active]:bg-background data-[state=active]:shadow-sm sm:text-base"
            >
              <LayoutGrid className="mr-1.5 h-4 w-4 shrink-0 sm:mr-2" aria-hidden />
              Teams
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="bracket">
            {tournament?.registeredTeams?.length < 2 ? (
              <TournamentTabEmpty
                ...
                description="At least two teams must register before the bracket can be shown."
                footer={
                ...
                <div className="rounded-lg border overflow-hidden">
                  <div className="overflow-x-auto">
                    <table className="w-full">
                      <thead className="sticky top-0 z-10 border-b bg-muted/80 backdrop-blur supports-[backdrop-filter]:bg-muted/60">
                        <tr>
              ...
              <div className="grid grid-cols-1 gap-4 sm:gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                {[...tournament.registeredTeams]
                  ...
                  return (
                    <Card
                      key={team._id}
                      className={cn(
                        "group flex flex-col overflow-hidden rounded-2xl border-2 border-primary/20 bg-gradient-to-br from-card via-card to-primary/5 shadow-sm transition-all hover:border-primary/30 hover:shadow-lg hover:shadow-primary/5",
                      )}
                    >
                        <div className="p-4 sm:p-5 flex flex-col flex-1">
                          ...
                          <Button
                            variant="outline"
                            size="sm"
                            className="mt-auto h-9 w-full rounded-full text-sm font-medium"
                            onClick={() =>
                ...
                {tournament.status === "upcoming" &&
                  Array.from({
                    ...
                    <Card
                      key={`empty-${index}`}
                      className="flex min-h-[150px] flex-col rounded-2xl border-2 border-dashed border-primary/25 bg-muted/10"
                    >
                      <div className="flex flex-1 flex-col items-center justify-center p-6 text-muted-foreground">
                        ...
                        <p className="text-center text-xs">Open slot</p>
                      </div>
                    </Card>
                  ))}
              </div>
        ...
        {(isAdmin || canManage) && tournament ? (
          <TournamentAdminControls
            ...
            onPreseedOrUndo={() => {
              void (isSeeded ? handleUndoSeeding() : handlePreseed());
            }}
            onLaunch={() => {
              void handleLaunchTournament();
            }}
            onReset={() => {
              void handleResetTournament();
            }}
            onOpenFillDialog={() => setIsFillDialogOpen(true)}
