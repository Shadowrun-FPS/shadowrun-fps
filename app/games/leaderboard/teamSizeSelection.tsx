...
export default function TeamSizeMenu() {
  ...
  const currentTeamSizeString = !searchParams.get("teamSize")
    ? teamSizeDefault
    : searchParams.get("teamSize");
  const currentTeamSizeHeadString =
    currentTeamSizeString + "v" + currentTeamSizeString;

  function handleClick(nextTeamSizeString: string) {
    if (currentTeamSizeString === nextTeamSizeString) return;
    router.push("/matches/leaderboard/?teamSize=" + nextTeamSizeString);
  }
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button
              variant="outline"
              size="default"
              className="h-full px-5 py-0 my-0 text-2xl"
              key={currentTeamSizeString}
            >
              ...
              <span className="sr-only">
                Select Leaderboard based on Team Size
              </span>
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="center">
            {teamSizeOptions.map((teamSize) => (
              <DropdownMenuItem
                ...
                onClick={() => handleClick(teamSize.toString())}
                key={teamSize}
              >
                {teamSize}v{teamSize}
              </DropdownMenuItem>
