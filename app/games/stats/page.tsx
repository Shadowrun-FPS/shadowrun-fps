...
async function playerSearch(searchString: string) {
  try {
    ...
    const players = await db.collection<Player>("Players").find({discordNickname:{'$regex' : searchString, '$options' : 'i'}}).limit(20).sort({discordNickname: 1}).toArray();
    return players;
  } catch (error) {
...
export default async function StatsPage({searchParams}: {searchParams: {search: string}}) {
