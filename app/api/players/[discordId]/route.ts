...
export async function POST(request: NextRequest, {params}: {params: {discordId: string}}) {
  ...
  const db = client.db("ShadowrunWeb");
  const discordId = params.discordId;
  const currentPlayerInfo = await db.collection("Players").findOne({discordId: discordId});

  if (!currentPlayerInfo) return NextResponse.json({
    ok: false,
    ...
    switch (action) {
      case('update'): {
        ...
        if (!discordNickname &&
            ...
            !newStat) return NextResponse.json({
          ok: false,
          message: "Data object must include discordNickname, discordProfilePicture, or stats object to update user's Discord Info",
          status: 400
        })
        if (!discordNickname) {
        ...
        await db.collection("Players").updateOne({discordId: discordId}, {
          $set: {discordNickname: discordNickname,
