  ...
  for (var i = 0; i * playersPerPage < playerCount; i += 1) {
    ...
    PageButtons.push(
      <Button
        size="sm"
        className={
          i === page - 1 ? "bg-slate-300" : "bg-slate-400 dark:bg-slate-600"
        }
  ...
  return (
    <>
      <div className="flex justify-center w-full">
        <div
          className={
            "flex content-center w-72" +
            (i == 1 ? " justify-center" : " justify-between")
          }
        >
          {PageButtons}
        </div>
      ...
      <div className="flex justify-center w-full">
        Showing {playersPerPage * (page - 1) + 1} -{" "}
        {Math.min(playerCount, playersPerPage * page)} of {playerCount}
      </div>
    </>
  );
}
