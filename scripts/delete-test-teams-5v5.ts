...
async function deleteTestTeams() {
  try {
    ...
    console.log("Starting 5v5 test team deletion...");

    // Find all 5v5 test teams (by prefix or isTestTeam flag with teamSize 5)
    ...
    console.log(`Found ${testTeams.length} 5v5 test teams to delete`);

    if (testTeams.length === 0) {
      console.log("No 5v5 test teams found to delete.");
      return;
    }
    ...
    console.log(`\n✅ Successfully deleted ${result.deletedCount} 5v5 test team(s)`);
    ...
    if (remainingTestTeams.length > 0) {
      console.log(`\n⚠️  Warning: Found ${remainingTestTeams.length} additional teams with 5v5 test prefix:`);
      ...
      console.log("\n✅ All 5v5 test teams have been deleted successfully!");
    }
  } catch (error) {
    console.error("Error deleting 5v5 test teams:", error);
    process.exit(1);
  }
}
