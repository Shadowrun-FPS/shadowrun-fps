...
async function calcTeamEloInline(teamId: string, client: MongoClient): Promise<number> {
  ...
  if (!team) {
    return DEFAULT_INDIVIDUAL_ELO * 4;
  }
...
async function createTestTeams() {
  try {
    ...
    for (let i = 0; i < NUM_TEAMS; i++) {
      ...
      // Ensure each player has stats with 4v4 ELO
      const members = [];
      for (const player of validPlayers) {
        // Get or create 4v4 stats
        let elo = getRandomElo();
        
        if (player.stats && Array.isArray(player.stats)) {
          const stat4v4 = player.stats.find((s: any) => s.teamSize === 4);
          if (stat4v4 && typeof stat4v4.elo === "number") {
            elo = stat4v4.elo;
          } else {
            // Add 4v4 stats if missing
      ...
      // Calculate initial team ELO (sum of top 4 players)
      ...
      const result = await db.collection("Teams").insertOne({
        name: teamName,
        description: `Test team for tournament testing (created ${new Date().toISOString()})`,
    ...
    console.log(`\nTo delete these teams, run: npm run delete-test-teams`);
  } catch (error) {
